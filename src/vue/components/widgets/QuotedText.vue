...
const firstSpanContent = computed(() => {
    if (hasLineBreaks.value) {
        ...
        if (words.length > 1) {
            ...
            return `<i class="fa-solid fa-quote-left text-primary me-1"></i><br/>${content}`
        }
        return `<i class="fa-solid fa-quote-left text-primary me-1"></i><br/>${text}`
    }
...
const lastSpanContent = computed(() => {
    if (hasLineBreaks.value) {
        return `${lastPart.value}<br/><i class="fa-solid fa-quote-right text-primary ms-1"></i>`
    }
